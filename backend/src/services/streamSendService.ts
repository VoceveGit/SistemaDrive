...
// Envio sob demanda: rebaixa o arquivo do Drive e insere em lotes (só quando o usuário clica Enviar).
...
/**
 * Rebaixa o XLSX do Drive e grava no MySQL em lotes.
 * Usado quando o preview estava truncado OU quando o usuário pede envio completo via stream.
 */
export async function streamSendFromDrive(params: {
  ...
  if (!company.targetTable) {
    throw new Error("Tabela destino não configurada");
  }
  ...
  try {
    await prisma.spreadsheet.update({
      ...
      data: {
        status: "processing",
        processMessage: "Enviando ao banco (streaming)...",
      },
    });
      ...
      onBatch: async (batch) => {
        ...
        await prisma.spreadsheet.update({
          ...
          data: {
            ...
            newRows: inserted,
            processMessage: `${inserted} linhas gravadas no banco...`,
          },
        });
      },
    });
